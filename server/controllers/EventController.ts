...
export const getEvents = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const events: EventStructured[] = response.results.map((page: any) => {
            ...
            const location = page.properties.Location.rich_text[0]?.text?.content || "Location to be confirmed";
    ...
    .sort((a, b) => new Date(a.eventStartDateTime).getTime() - new Date(b.eventStartDateTime).getTime());

    const pastEvents = events
    .filter(event => new Date(event.eventStartDateTime) < now)
    .sort((a, b) => new Date(b.eventStartDateTime).getTime() - new Date(a.eventStartDateTime).getTime());
