import { SponsorCardProps } from "../utils/FrontendTypes";
  ...
  <div className="w-full h-full relative rounded-3xl bg-white">
        ...
        <div className="w-2/3 flex flex-col items-start justify-between py-2 pr-2 relative">
          <div>
            <h2 className="text-lg sm:text-2xl md:text-xl xl:text-xl font-bold leading-tight">{name}</h2>
            <p className="text-sm sm:text-base md:text-lg 2xl:text-base font-semibold opacity-90 mb-2">{location}</p>
            <p className="text-xs sm:text-base opacity-90 line-clamp-2 sm:line-clamp-3">
                {deal}
                </p>
          </div>
          ...
          <a
            href={link}
            className="absolute bottom-2 right-2  px-4 py-1 md:px-3 md:py-1 lg:px-5 text-lg md:text-lg rounded-full font-bold transition-all hover:opacity-90 duration-300 bg-white text-red"
            >
            Website
            </a>
        </div>
